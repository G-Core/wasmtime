            ...
            Some(inputs) => {
                ...
                for input in &inputs {
                    let id = input
                        ...
                        .parse::<u32>()
                        .map(|v| Id::Index(v))
                        .unwrap_or_else(|_| Id::Name(input.name.clone()));
                    self.set_input(id, &input.tensor)?;
                }
                ...
                // Get all outputs
                let output_count = self.0.get_outputs_len()?;

                let mut output_tensors = Vec::new();
                for i in 0..output_count {
                    let tensor = self.get_output(Id::Index(i as u32))?;
